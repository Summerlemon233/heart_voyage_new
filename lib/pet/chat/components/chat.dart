                  ...
                  if (isShownPrompts) {
                    return GetX<PromptController>(builder: ((controller) {
                      if (controller.prompts.isEmpty) {
                        return const Center(
                          child: Center(child: Text("正在加载提示词...")),
                        );
                        ...
                        return const Center(child: Text("加载提示词列表失败，请检查网络"));
                      }
                    }));
                  } else {
                    return Center(
                      child: TextButton(
                        child: Text("点击从网络加载提示词列表（不稳定）。"),
                        onPressed: () {
                          setState(() {
                            isShownPrompts = true;
                          });
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Avatar(image: AssetImage('assets/images/rabbit.png')),
              const SizedBox(
                width: 8,
              ),
              Text(message.role == Role.system ? "系统" : "心宠"),
            ],
          ),
          Row(
            mainAxisAlignment: MainAxisAlignment.start,
